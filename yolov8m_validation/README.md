## Setup a virtualenv in Ubuntu24.04 and install amd-quark 0.10

### 1. Install cmake
```
sudo apt install cmake=3.28.3-1build7 python3-pip
```

### 2. Setup a virtualenv
```
pip install virtualenv virtualenvwrapper --break-system-packages
```
...
```
### 4. Download [amd-quark0.10 package](https://download.amd.com/opendownload/Quark/amd_quark-@version@.zip)
```
pip install torch torchvision --index-url https://download.pytorch.org/whl/rocm6.4
pip install ./amd_quark-0.10-py3-none-any.whl
```

## Run direct benchmark
### 1. Install revelant YoloV8m pip library
```
...
```
hf download amd/yolov8m yolov8m.onnx --local-dir .
python benchmark_onnx.py yolov8m.onnx --onnx_ep npu --test_num 600 --parallel 4
```
...
If yolo automatically install `onnxruntime`, need to uinstall it to avoid conflict with RyzenAI
...
```
python benchmark_onnx.py yolov8m_int8.onnx --onnx_ep npu --test_num 600 --parallel 4
```
